...
int* Dijsktra(int startId, int endId, Grafo& grafo, int& size) {
    ...
    float* costsSoFar = new float[graphSize];
    for (int i = 0; i < graphSize; i++) {
        costsSoFar[i] = FLT_MAX;
    }
...
void InsertNeighbors(PriorityQueue &queue, int graphSize, NodeCosts& currentNode, Grafo& grafo, int*& parents, float* costSoFar) {
    float currentCost;
    for (int i = 0; i < graphSize; i++) {
